...
public class UserDaoImpl implements UserDao {
	...
	public static final String UPDATE_SQL="update user_tbl set user_name=?,password=?,email=?,gender=?,dob=?,hobbies=?,nationality=?";
	...
	public User getUserInfoById(int id) {
		...
		try (
			Connection con= DbUtil.getConnection();
			PreparedStatement ps= con.prepareStatement(SELECT_SQL_BY_ID);){
			ps.setInt(1, id);
			ResultSet rs= ps.executeQuery();
			if(rs.next()){
